// Snapshot of Microsoft Entra built-in role template IDs, reviewed 2026-09-19.
...
const entries = [
  ...
  ['Entra SOC Identity Responder', '58f930cc-fcf4-4152-852c-1d7dbf502139'],
...
export const tailoredRoleNames = new Set([
  'Global Administrator', 'Security Administrator', 'Global Reader', 'Intune Administrator',
  ...
  'Cloud Application Administrator', 'Identity Governance Administrator',
]);

const normalize = value => value.trim().replace(/\s+/g, ' ').toLowerCase();

export const roles = entries.map(([name, templateId]) => ({
  name,
  templateId,
  aliases: aliases.filter(([, canonical]) => canonical === name).map(([alias]) => alias),
  tailored: tailoredRoleNames.has(name),
}));

const byName = new Map(roles.map(role => [normalize(role.name), role]));
const byId = new Map(roles.map(role => [role.templateId, role]));
const aliasByName = new Map(aliases.map(([alias, canonical]) => [normalize(alias), canonical]));

export function resolveRole(value) {
  const normalized = normalize(value);
  const canonical = aliasByName.get(normalized);
  return byId.get(normalized) || byName.get(canonical ? normalize(canonical) : normalized) || null;
}
